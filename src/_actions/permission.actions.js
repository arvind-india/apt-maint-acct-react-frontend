...
import { permissionService as service } from '../_services'
  ...
  return dispatch => {
    ...
    service.getAll()
      .then(
        ...
        error => dispatch(failure(error+' getting all permission models'))
      )
  }
  ...
  return dispatch => {
    ...
    service.getById(id)
      .then(
        ...
        error => dispatch(failure(error+' in get permission model by id: '+id))
      )
  }
      ...
      service.add(model)
        .then(
          model => {
            dispatch(success())
            history.push('/permissions')
            dispatch(alertActions.success('Added new Permission Successfully'))
          },
          error => {
            let data = error.response.data
            // console.log('error response...')
