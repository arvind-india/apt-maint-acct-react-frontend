...
class RoleDetailsPage extends React.Component {
  ...
  componentDidMount() {
    this.props.dispatch(roleActions.getById(this.props.match.params.id))
  }
  changedProps() {
    const { mUser, mInfos, mResidentType } = this.state
    const { model } = this.props
    let userDB = model.data
  ...
  handleSubmit(event) {
    ...
    if ( cProps.length == 0 ) {
      ...
      dispatch(roleActions.saveChanges(mUser))
    } else {
  ...
  render() {
    const { model, user, match, alert, submitted } = this.props
    return (
      <div>
        <h2>User Details</h2>
...
function mapStateToProps(state) {
  const { model, authentication, alert } = state
